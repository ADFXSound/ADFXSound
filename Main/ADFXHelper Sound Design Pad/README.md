...
## v0.8.43 - Native Project Playback Rate
- Adds `+ PROJECT RATE` to each X/Y/Z/Tilt assignment bus.
- Project Playback Rate is a native REAPER project target, not an FX parameter.
- Uses REAPER's playback-rate normalization so existing 0-1 mapping, CENTERED/BIPOLAR, invert, range preview, and LFO routing work with playback rate.
- Default centered range is 0.25x -> current project rate -> 2.00x.
- Project Playback Rate is global to the project; it is not per-track.
